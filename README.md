...
| Serena·Sequential Thinking | 외부 BAT가 설치·설정·검증하고, 런타임에서는 권장 도구로 유지 |
...
| Skill | 역할 |
| --- | --- |
| `$serena-project-setup` | 설치 후 Serena 프로젝트·인덱스·memory·공유 HTTP 서비스의 복구·점검 |
...
초기 설치는 Skill이 아니라 일반 terminal에서 실행하는 `scripts\install_mcp_dependencies.bat`와 `scripts\setup_agent_team.bat`가 맡는다.
...
## MCP 설치·설정과 런타임 권장 정책

Serena와 Sequential Thinking은 activation packet 안에 계속 남는 **권장 도구**다. 다만 최초 설치·재설치에서는 두 MCP를 반드시 준비해야 하므로, 일반 terminal의 설치 배치가 둘 다 설치·설정·검증한다. 설치가 끝난 뒤 일시적인 런타임 장애는 core initialization이나 현재 작업을 자동으로 `BLOCKED`로 만들지 않는다.
...
`setup_agent_team.bat`이 Serena 프로젝트 생성·인덱싱·health check·memory 초기화와 공유 loopback Streamable HTTP 서비스를 준비한다. 서비스는 `127.0.0.1`의 무작위 영속 포트와 `/mcp` endpoint를 쓰며, 좌석마다 stdio 서버를 새로 띄우지 않는다.
...
생성되는 MCP block은 설정 배치가 두 의존성을 확인한 뒤 `.codex/config.toml`에 넣으며 항상 `required = false`다. 즉 설치 성공 조건과 런타임 강제 계약은 구분한다.

## 설치와 초기화

전달 번들을 대상 프로젝트에 overlay한 뒤, 아래 순서는 **일반 PowerShell 또는 Command Prompt**에서 대상 프로젝트 루트로 이동해 실행한다. Codex 세션 안에서 실행하지 않는다.

```powershell
Set-Location <target-project> # e.g. C:\project\manager\sdetector-manager

# 1. MCP executable/package installation only
.\scripts\install_mcp_dependencies.bat --dry-run
.\scripts\install_mcp_dependencies.bat

# 2. Project-local Serena and Codex configuration only
.\scripts\setup_agent_team.bat --dry-run
.\scripts\setup_agent_team.bat
```

`install_mcp_dependencies.bat`는 Python·npm을 확인하고 Serena CLI가 없을 때만 일반 사용자 terminal에서 `uv tool install -p 3.13 serena-agent`를 실행한다. 이어서 정확히 고정된 Sequential Thinking 패키지를 대상 프로젝트의 `.agent-team/mcp`에 설치하고 엄격히 확인한다. 이 단계는 `.codex/config.toml`, `.serena/project.yml`, Serena HTTP 서비스를 만들지 않으며 `serena init` 또는 `serena config edit`도 실행하지 않는다.

`setup_agent_team.bat`는 위 의존성 검사가 통과한 경우에만 core initialization, `.serena` 생성·인덱싱·memory 초기화, 공유 HTTP 서비스 시작, 두 MCP 설정·엄격 검증, 최종 core 검증을 수행한다. 이 단계는 누락된 MCP 패키지를 설치하지 않는다. 설치 배치가 실패했으면 설정 배치를 실행하지 않는다.

core initialization은 필요한 Python 의존성 확인, Skill 검증, 좌석 registry 생성 또는 보존, `.codex/agents/` 동기화, 프로젝트 로컬 `.codex/config.toml` 생성, `.agent-team/state` 초기화를 수행한다. 생성되는 설정은 `workspace-write`, `approval_policy = "on-request"`, 추가 writable root 없음, network off를 요청한다. 이후 대상 프로젝트를 신뢰하고 Codex를 reload/restart해야 생성된 config와 좌석을 읽는다. 단, 호스트가 `.codex/`를 보호 경로로 유지하면 이 설정이 그 보호를 우회하지는 않는다.

Serena endpoint만 다시 잡아야 하는 복구 작업에는 다음 명령을 쓴다.
...
- `project_skills.py validate`: project-local Skills 23개 통과(bootstrap Skill 제거);
...
- `install_mcp_dependencies.bat`와 `setup_agent_team.bat` dry-run이 각각 의존성 설치와 프로젝트 설정의 서로 다른 순서만 출력하는 것 검증;
- dependency install은 Codex MCP config를 만들지 않고, setup configuration은 Sequential Thinking 설치 함수를 호출하지 않는 것 검증;
...
```text
d9767c296170becc9baf25d3fd60c2832c3c6a78ff0da62b8f2ec6b2282e9611
```
