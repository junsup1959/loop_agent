...
def _render_codex_config(
    ...
    lines = [
        ...
        "# This project-local default does not override host-managed protected paths.",
...
def _check_codex_config(
    ...
    if config.get("approval_policy") != "on-request":
        raise InitializationError("Project Codex config must set approval_policy to on-request.")
    if config.get("sandbox_mode") != "workspace-write":
        raise InitializationError("Project Codex config must set sandbox_mode to workspace-write.")
    sandbox_workspace_write = config.get("sandbox_workspace_write")
    if (
        not isinstance(sandbox_workspace_write, dict)
        or sandbox_workspace_write.get("writable_roots") != []
        or sandbox_workspace_write.get("network_access") is not False
    ):
        raise InitializationError(
            "Project Codex config must keep the workspace-write sandbox project-scoped."
        )
