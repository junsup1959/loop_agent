...
TEAM_PATH = PROJECT_ROOT / "config" / "agent-team" / "team.toml"
...
def _load_context_document(path: Path, max_chars: Any, *, label: str) -> dict[str, Any]:
    if not path.is_file():
        raise AgentConfigurationError(f"{label} file not found: {path}")
    if not isinstance(max_chars, int) or max_chars <= 0:
        raise AgentConfigurationError(f"{label} character limit must be a positive integer.")
    content = _require_english(path, label, _read_utf8(path)).strip()
    if len(content) > max_chars:
        raise AgentConfigurationError(
            f"{label} exceed the configured {max_chars}-character limit."
        )
...
def _scan_source_policy(paths: list[Path]) -> None:
    ...
    agents_root = PROJECT_ROOT / "config" / "agent-team"
    references_dirs = [
...
def _load_static_configuration() -> dict[str, Any]:
    ...
    activation_instructions = _load_context_document(
        activation_instructions_path,
        team.get("max_activation_instruction_chars"),
        label="activation instructions",
    )
    recommended_tools_path = _configured_path(
        team.get("recommended_tools"), "recommended_tools"
    )
    recommended_tools = _load_context_document(
        recommended_tools_path,
        team.get("max_recommended_tool_chars"),
        label="recommended tools",
    )
    if source_root != (PROJECT_ROOT / "config" / "agent-team").resolve():
        raise AgentConfigurationError(
            "Canonical agent configuration must be config/agent-team/."
        )
    if runtime_root != (PROJECT_ROOT / ".codex" / "agents").resolve():
        raise AgentConfigurationError("Runtime agent root must be .codex/agents/.")
    if skill_catalog_path != (
        PROJECT_ROOT / ".agents" / "skills" / "catalog.toml"
    ).resolve():
        raise AgentConfigurationError(
            "Agent configuration must use the native project skill catalog."
        )
