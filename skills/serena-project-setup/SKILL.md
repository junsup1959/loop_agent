...
description: Prepare a local project for a shared Serena Streamable HTTP MCP service before an agent team starts. Use when registering or repairing a Serena project, selecting language support, indexing and health-checking source analysis, initializing Serena memories, configuring contexts or modes, or starting or rotating the project-local shared Serena service endpoint.
---

# Serena Project Setup

Configure Serena deliberately before the agent team is initialized. Keep all agents on one local project and one shared Streamable HTTP server; do not let each spawned agent start a separate stdio server.
...
Use `agents/serena-service.toml` as the project-local service contract. Start the server through the bundled service manager so it chooses an available loopback port, persists the endpoint in `.agent-team/state/serena-service.json`, and prevents agent spawn from creating a new MCP process.

```powershell
python .\skills\serena-project-setup\scripts\manage_serena_service.py `
  --service-config .\agents\serena-service.toml start `
  --project <target-project>
```

The command repeats the Serena health check, selects a random available port, and persists both the concrete endpoint and its successful health record. It refuses to start on a failed health check. Do not generate Codex MCP configuration from this setup skill during a first installation; `$agent-team-bootstrap` runs the full initializer only after this step succeeds.
