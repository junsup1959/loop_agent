...
Serena와 Sequential Thinking은 팀의 강제 계약이 아니라, 매 활성화에 명시적으로 주입되는 **권장 도구 컨텍스트**다. 권장 도구 문서는 MCP가 설치되지 않았거나 일시적으로 사용할 수 없어도 패킷에서 사라지지 않는다.
...
기본 초기화는 MCP를 켜지 않는다. 활성 상태와 마지막으로 확인된 Serena URL은 `.agent-team/state/mcp-capabilities.json`에 저장한다.

```powershell
# Explicitly enable a recommended capability.
python .\scripts\init_agent_team.py --enable-mcp serena
python .\scripts\init_agent_team.py --enable-mcp sequentialthinking

# Strictly check only the capability requested here.
python .\scripts\init_agent_team.py --check-mcp serena

# Remove a capability from generated Codex configuration without deleting its data.
python .\scripts\init_agent_team.py --disable-mcp serena
```

활성화된 MCP만 `.codex/config.toml`에 생성되며 둘 다 `required = false`를 사용한다. 일반 `init`과 `--check`는 MCP 준비 상태를 점검하거나 MCP 장애 때문에 실패하지 않는다. MCP만 엄격히 확인하려면 `--check-mcp`를 사용한다.
...
Serena를 사용하려는 경우에만 `$serena-project-setup`을 실행해 프로젝트 설정, 인덱스, 메모리, loopback Streamable HTTP 서비스를 준비한 뒤 `--enable-mcp serena`을 실행한다.
